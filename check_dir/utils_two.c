...
/*   Updated: 2024/10/24 17:27:57 by npizzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    char *possible_instructions[] = {"ra\n", "rra\n", "sa\n", "pa\n", "rb\n", "rrb\n", "sb\n", "pb\n", "rr\n", "rrr\n", "ss\n"};
    int i;

    i = 0;
    while (i <= 10)
    {
        if ((ft_strncmp(instruction, possible_instructions[i], ft_strlen(instruction))) == 0)
            break;
        i++;
    }
    if (i == 11)
...
void check_for_segfault(int instruction_number, char *instruction, t_stack **stack_a, t_stack **stack_b)
{
    ...
    segfault = false;
    if (*stack_a == NULL && (instruction_number <= 2 || instruction_number >= 7))
        segfault = true;
    if (*stack_b == NULL && ((instruction_number >= 3 && instruction_number <= 6) || instruction_number >= 8))
        segfault = true;
    ...
    {
        if (((instruction_number >= 4 && instruction_number <= 6) || instruction_number >= 8))
            segfault = true;
    }
    if (segfault == true)
        free_all_and_exit(instruction, stack_a, stack_b);
