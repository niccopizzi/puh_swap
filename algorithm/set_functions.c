...
/*   Updated: 2024/10/24 17:36:24 by npizzi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void set_cost(t_stack **stack_a, t_stack **stack_b)
{
    ...
    while (ptr_b != NULL)
    {
        if (ptr_b->above_median == true && ptr_b->target->above_median == true)
            ptr_b->cost = ft_max(len_a - ptr_b->target->position, len_b - ptr_b->position);
        else if (ptr_b->above_median == false && ptr_b->target->above_median == false)
            ptr_b->cost = ft_max(ptr_b->target->position, ptr_b->position);
        else
        {
            ptr_b->cost = ft_min(ptr_b->target->position, len_a - ptr_b->target->position);
